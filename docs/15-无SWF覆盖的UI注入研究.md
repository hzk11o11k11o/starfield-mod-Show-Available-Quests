...
**待实机（判据）**：
- ① 正常态（我们的 SWF）：主计划 **43 条** —— 新增 `r125_ui_channel_ok`（推送成功且
  **没有**「UI 通道不可用」行 —— 防误报；日志里不应出现该行）；
- ② P2 态（原版 SWF，下次 P2 会话）：P2 计划 **2 条** —— 新增 `r125_ui_conflict`：
  `UI 通道不可用（第 N 次推送失败后判定）` 出现 + **不再出现**「推送放弃：重试 14 次」
  （停推降噪生效）；眼睛 = 关菜单后 HUD 出现提示（菜单关闭后的第一个轮询节拍）；
  日志一行 `UI 通道提示已请求`（进程内首判时）。
