...
- 余项 = **P2 实验态 6 条**（r137 断言含 `指纹=ok` —— 结构指纹自检的实机证据）——
  **P2 态已重新部署**（`*.p2off` + P2 计划（含 `指纹=ok` 断言，工作区 == 部署）+
  `UiMode=auto`；`verify --dev --p2` **868 行 / 0 MISS**）。

### 15.5 打包 0.1.16

见 15.4 的 P2 态 6 条实机判读之后执行；本版新增内容 = 双形态发布（冲突自动切注入 + 结构指纹自检）；
文案更新点 = README-mod.txt / nexus-description.md 的「界面未生效」段（从「只提示原因」
升级为「自动切换到注入形态、功能照常可用」）+ changelog v0.1.16。
