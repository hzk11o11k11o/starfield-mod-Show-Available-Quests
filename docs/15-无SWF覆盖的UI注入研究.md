...
**眼睛（待玩家确认）**：30 秒窗口（19:57:57.373 再注入 → 19:58:27 menu.close）第 8 个
tab「可接任务」+ **真实 206 条列表**（真任务名/描述；切回原版 tab = 原版内容）。

**结论**：P3-a（真实数据注入）**判据收口** —— 「分时注入」闭环（切到我们 tab ⇒ 注入
206 条 / 切走 ⇒ 完整恢复引擎列表）在「无 SWF 覆盖」形态下**产品级成立**。
下一步 = **P3-b**（完整 `SaqDescriptionText` 文案迁移 + watchdog + `UiMode` 运行期开关）
⇒ P4 交互接管。
