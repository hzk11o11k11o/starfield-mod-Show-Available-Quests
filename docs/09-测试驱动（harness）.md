...
## 十四·补十六 ~ 补十八、第 83 / 84 / 85 轮（明细已迁 docs/94）

> 第 160 轮瘦身：三条原文**逐字迁入** `docs/94` 的「第 160 轮迁入」节（只剪切、不改写）。
> 仍然生效的结论：`ui.*` 桥未通在步骤超时内自动重试 + 清场延迟复查判据 + 断言超时三点打点
> + `plan_regex_audit`（第 83 轮）；结果 JSON 非法 UTF-8 容错读取 + `log_hygiene`（第 84 轮）；
> 22/22 全 PASS 实证（第 85 轮）。
...
* ★ **复跑收口（第 160 轮 · 07:18:35~07:20:44 会话 / `sessionMs` 88265）**：两道闸实证成立 ——
  `07:19:42.581 harness：等 世界还没落地（FaderMenu 开着…）` → `07:19:43.587 harness：
  静态表已主动就绪（本会话第一次用到表…）` → 0.02 s 后开始用例；`teleport.entry` 的
  `参数解析` 行 = `任务板自身（0x00015062）｜任务板自身[命中]`；r155 三条断言 + 六条反向
  全过 ⇒ **转 PASS**（`check_results` 1/1 / `log_hygiene` 0 / 窗口自检 0 问题 / `verify --dev`）。
  沉淀：**通道就绪 ≠ 世界落地 ≠ 表已建**，三者都是开跑前置。

### 7. 全量 45 条收口 + 0.1.19 打包（第 161 轮；含一次 verify 侧缺陷）

> 会话（2026-09-25 07:57:20~08:07:21 / `sessionMs` 600297 / **全量 45 条** / 驱动器 v74）：
> **判据五连全绿** —— `check_results` **45/45**（PASS 45 / FAIL 0 / SKIP 0）+ `log_hygiene`
> 退出码 0（日志 2428 行 / 1358476 B 零坏字节 + 结果 JSON 1132 行干净）+ `plan_regex_audit`
> **174 断言全命中 / 窗口自检 0 问题**（4 条静态提示 = 已知异步用法）+ `verify --dev`
> **1159 行 / 0 MISS**（工作区 == 部署）+ 离线层 5 步全过；`[E]` **0** / `[W]` **49**
> （48 条「推送失败（第 1 次）」+ 1 条 UI 桥 = 两组老现象）。

* 就绪门两道闸再次实证：`07:58:31.169 等 世界还没落地（FaderMenu 开着…）` →
  `07:58:32.178 静态表已主动就绪（本会话第一次用到表…）` → 0.02 s 后开始用例；r155 = PASS
  （33657 ms），判定行 = `船员探针 汇总：24 位｜Papyrus 读到 12 位（A=1 0 / C=1 0 / P=1 4 /
  已分配 0）｜直读 12 位（对照 12 位 / 不一致 0 位）｜招募任务 未开始 17 / 运行中 7 /
  已完成 0` + `船员直读：槽 0x174 指纹=ok` + 模拟招募判定行（A 判据无正样本，按第 156 轮口径保留）。
* 第一次启动（07:51:42 / 线程 `[26396]`）只载入用例、未开跑（玩家重启游戏）⇒ 结果 JSON 只含
  07:57:20 那一段（`sessionMs` 600297 与窗口精确对上）；两段都无 `[E]`。
* ★★★ **打包被挡一次 = verify 侧缺陷（产品零变化）**：0.1.19 首次 `package-saq.ps1` 在
  `verify --release` 中止 —— MISS「发布构建不含 harness 特征（泄漏：harness 船员探针数据表
  (样本名1)、harness 船员探针数据表(样本名2)）」。真因 = 那两条反向检查的特征串是**船员名字**
  （`林监工` / `瓦斯科`），而第 156 轮起这两个名字已进**产品**入口表（`（可招募）林监工` /
  `（可招募）瓦斯科`）—— 子串匹配对「产品长串里含短串」照样命中（实测发布 DLL：
  `（可招募）瓦斯科` 在场、`crew.probe` / `船员探针 {}` 不在场）。修法（照第 144 轮先例）：
  这两条移出 `HARNESS_STRINGS`，换成两条**只属于探针**的文案（`船员探针 {}` 逐位行前缀 /
  `船员探针没有跑完` 超时文案）；「数据表在不在」不再做字符串判据（表只有名字 + 数字，
  名字与产品数据必然重合）——「产品侧船员数据在」由产品表的名字前缀
  （`（可招募）玛丽卡·波罗斯`）正向覆盖。改后 `verify --release` **836 行 / 0 MISS**、
  `verify --dev` **1159 行 / 0 MISS**。
* 打包与还原：`dist\SAQ-ShowAvailableQuests-0.1.19.zip`（**1,891,594 B** / SHA256
  `42D4E1AC…9B6D60E`；发布 DLL **932864 B** / `DF803BEF…2C776D13`；7 文件 = README.txt /
  ESM 4805 / DLL 932864 / SWF 733596 + lrg 733707 / PEX 21568 / ini 7121）；
  `verify --release` 全过 + **打包后补跑 verify 覆盖新包**；包内 ini = 玩家默认值
  （Mode=0 / Harness=0 / AutoLoad 空 / MaxSizeMB=1）；打完后还原常规部署（开发构建
  **1248768 B** / `0D243566…E210F1`（工作区 == MO2）+ `Harness=0` / `AutoLoad` 清空 /
  `MaxSizeMB=10`）。
