...
* 重进游戏（`Harness=1` + `AutoLoad=Save1_…142854_2_0_4`）⇒ 期望 **35/35**；
  另确认 `ui.select 0x0021625F`（防御措施）/ `0x001A8B64`（登陆不顺）两条显示证据
  （本次会话在该断言 FAIL 后用例中止、未执行到）；
* 眼睛三项（`docs/13` 第六节）：星际盗贼图标 / 引导落阿基拉城总部 / 列表三条；
* 通过后打 **0.1.13**。
