...
## 十四·补二十四、第 98 轮：DLC 链式门槛的两条用例（`r98_dlc_chain` / `_pass`）

> 背景：DLC 不发 `.psc` ⇒ 破碎空间主线后续（虚妄的得诺者 / 家族的调和 / 狂热逼界 /
> 信念之争 / 发觉过去 / 栉比堡垒）此前只能按 INFO 门槛「保守放行」。第 98 轮反编译
> `.pex` 挖出 6 条真实启动边（`tools/esm/gen_dlc_chain.py` → `kChainGates`；
> 取证全过程见 `docs/06` 第八节）。

* **A 段 `[case:r98_dlc_chain]`**：七条 `quest.reset ~0x…`（MQ01 + 六条后续）⇒ `menu.open`
  ⇒ 断 `链式门槛=\d+\(过\d+/藏\d+/未知\d+`（`scope=case`）+ **六条任一**出现在
  `链式没到:` 名单里（`.*(名1|名2|…)\[0x`，`scope=case`）。
  ★ 为什么「或」列六条：测试存档的 DLC 进度未知，且 INFO 门槛在链式门槛**之前**判定 ——
  只要有一条没被 INFO 先藏就会进链式名单（离线预判 `ref/_dlc_info_precheck.py`：
  六条的 10/12/9/11/22/13 个对话组里分别有 6/3/3/2/9/3 组「可能可用」⇒ 都不会被 INFO 藏）。
  ★ **名字后面只写 `[0x`，不写死 FormID**：DLC 运行期 FormID 带加载前缀（本机
  ShatteredSpace = 0x01，换机器/换顺序就变）—— 这正是 `~0x…` 记录号写法存在的理由。
* **B 段 `[case:r98_dlc_chain_pass]`**：`quest.start ~0x000121DB` + `quest.stage … 10000`
  （MQ01 的收尾 fragment；它本身会顺手 `MQ02.SetStage(100)`）⇒ 断
  `assert.nolog 链式没到:.*虚妄的得诺者 scope=case`。
  ★ 与 `r67_chain_pass` 同款说明：边一触发 MQ02 就被引擎开始（running）⇒ 之后由运行时
  状态过滤接手（不在列表里是正常的）⇒ B 段证的是「**不再因链式门槛**被藏」，不是
  「出现在列表里」（那要多一个「链路真跑通 + 没被运行时过滤」的前提，不在此重复）。
* 两条用例自清场（把 MQ01 与六条后续 `quest.reset` 回去）；只动这 7 条任务的状态，
  不传送、不开星图、不读档；插在 `r62_reload_observe`（读档用例）**之前**。
* verify 侧同步 +3 条检查（两个 `[case:]` + 断言形状 + 「不许写死运行期 FormID」反向检查）。
* **待实机**：跑完即为「DLC 收口」的实机验收（随后 0.1.10 打包）。
