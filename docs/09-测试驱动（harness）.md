...
## 十四·补二十六、第 102 轮（下）：实机 32 条 **30 PASS / 2 FAIL** 的判读 + B 段改判

> 会话：2026-09-22 21:14~21:27（479750 ms，`驱动器 v70`，`stamp=63`）。
> 汇总：**32 条 30 PASS / 2 FAIL** —— 两条 FAIL 同源，都是
> `assert.nolog 链式没到:.*另一边`（推 `MQ03@4000` / `MQ05@1600` 后「另一边」仍在名单）。
>
> **判读：根因在写侧（引擎静默忽略），产品行为正确。** 三条硬证据（同一会话内）：
> ① **探针**（只读、与链式判定同源的引擎函数）：
>    `quest.probe 0x01030C2B：未开始 flags=0x10100｜stageDone(100)=0｜stageDone(3350)=0｜stageDone(4000)=0`；
>    对照段 `0x01035E1C` 同款（100/1000/1600/2000 全 0）；`MQIN(0x0110AAD5) stageDone(10)=0`
>    （fragment 没跑过 ⇒ MQIN 也没被启动）⇒ 引擎里 done 位**始终为 0**。
> ② **统计零变化**：`已开始=40` / `链式门槛=65(过4/藏59/未知0)` 在推之前与推之后**逐字段一致**。
> ③ **Papyrus 侧**：`seq=136/137/138 结果=0 —— Reset / Start / SetStage [SFBGS001_MQ03] => 4000`
>    （三条调用都「成功」）但**没有任何引擎侧脚本活动**；同一会话 r98 B 段推 `MQ_Shell@100`
>    时是**满屏 fragment 报错 + 已开始 34→44 + 链式 过4→7**（对照组成立：写侧通路本身没问题）。
> ⇒ 结论：**没进入运行状态的 ShatteredSpace 主线任务，Papyrus `Quest.Start()` /
>   `Quest.SetStage()` 被引擎静默忽略**（与第 99 轮 `MQ01@10000` 同族）。既然引擎状态没到，
>   产品继续把「另一边」藏在「链式没到」名单里 = **正确行为**，不是 bug。
>   （另：`quest.probe` 探针对 MQ03/MQ05/MQIN 的 `未开始 flags=0x10100` 说明
>   `Reset()` 之后这两条任务连 `Start()` 都没进去 —— 探针这个新能力正好被用上。）
>
> **处置（改判，不删用例）**：两条 B 段改成**走游戏自己的路径** ——
> 先 `reset` 五条（MQIN/MQ03/MQ04/MQ05/MQ_Shell）→ `start` + `stage MQ_Shell 100`
> （开场 fragment 对 MQ03/MQ04/MQ05 各 `SetStage(100)` ⇒ 三条一起进入运行；这段与
> r98 B 段**完全同源、已实测有效**）→ 再用 `quest.probe` 记下 `stageDone(100)=1`
> （= 构造成立的硬判据）→ 这时才推目标 stage（`MQ03@4000` / `MQ05@1600`）→
> `quest.probe` 记下目标 stage → 最后才 `assert.nolog 链式没到:.*另一边`。
> **断言链**：`stageDone(100)=1` → `stageDone(4000/1600)=1` → 放行；任一步红都能对号入座。
> ★ 配套：r98 B 段顺带 +2 条只读探针（`quest.probe ~0x00030C2B 100` /
>   `~0x00035E1C 100`）—— 它那段预热就是 B 段的构造前提，顺手把「fragment 的 SetStage(100)
>   是否真落到引擎」记进每次会话的日志。
> ★ verify：+7 条检查（预热步骤 / 两条探针断言 ×2 段 / r98 预热探针）+ **1 条反向检查**
>   （`quest.start ~0x00030C2B` / `~0x00035E1C` 不许再出现在用例里 —— 防改回不可达写法）。
> ★ 离线层 18 用例 / 929 断言 + 15 件快照全过（本轮只动用例与 verify，数据零变化）；
>   构建部署（`Harness=1`）+ verify 全过（含「MO2 部署 == 工作区」）。
>
> **兜底**：若下次实测 `stageDone(100)=0`（= 连 fragment 的 `SetStage` 都推不动这两条
> 主线任务）⇒ 说明本机引擎对 ShatteredSpace 主线任务的写入**完全不可用** ⇒ 这两条 B 段
> 按「记录型用例」改判（只断言 probe 事实 + 由 r98 B 段覆盖同类放行机制），不再追放行。
>
> **日志健康（本轮会话）**：`[E]` **0** 条；`[W]` 41 条 = 35 条「推送失败（第 1 次）」
> （老现象：菜单刚开那一拍 ASMovieRoot 还没解析出来，重试即成功 —— 每条用例的
> `assert.log 推送成功` 都过）+ 1 条「UI 桥解析失败」（同上，第一次的伴生行）+
> 1 条「星图：12.0 秒内没有打开」（**smoke 用例的 R 键**留下的星图待办：紧接着的 r26
> 用例把游戏按在暂停菜单上 27 秒 ⇒ 脚本的轮询节拍不走 ⇒ 12 秒后 DLL 打这条诊断；
> 星图本身的判据在 `r44_starmap`，本轮 **PASS**）+ 4 条 harness FAIL 行（= 上面两条 FAIL）。
> `check_results.py` 退出码 1（有 FAIL）、`log_hygiene.py` 退出码 0（证据通道干净）。
>
> **待实机判据**：32 条全 PASS + 两段 `quest.probe` 行给出 `stageDone(100)=1`、
> `stageDone(4000/1600)=1` + `check_results.py` / `log_hygiene.py` 退出码 0。
> 通过后 ⇒ 重打 **0.1.11** 上传包。
