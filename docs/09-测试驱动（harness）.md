...
## 十四·补二十一、第 92 轮（2026-09-22 首跑复查）—— 已由补二十二证伪/收口（速查）

> 26 条 24 PASS / 2 FAIL，两条 FAIL（r89/r90）全在用例侧：唯一失败步骤 = `assert.ui rptk=…`
> （`rptk=[0|-]`），同批 C++ 侧 `可重复任务=20(已完成保留1/2)` + 保留名单**全过**。
> 当时的真因判断 = 「`CompleteQuest()` 只动完成位、不进玩家日志」⇒ 处置 = 用例改
> 「先 `quest.start` → 再 `quest.complete`」+ 新增中间断言 `qdata=…<uID>`。
> ★ 该假设已被第 93 轮复跑**第二次证伪**（`Start` 也不进日志；引擎行为 = 完成即移除）——
> 原四节（现场 / 真因 / 处置 / 两种结局的判读与拟定红线）以本速查替代，
...
## 十四·补二十三、第 94 轮（2026-09-22 全量复跑）：**26 条 26/26 全 PASS** —— 第 89~91 轮产物实机收口

### 1. 现场

* 12:00~12:08 会话（407266 ms / `驱动器 v70`）；`check_results.py` / `log_hygiene.py`
  退出码均 0（日志 316472 B / 结果 JSON 283080 B，无非法 UTF-8 / 控制字符）；
* `[E]` **0** / `[W]` **13** —— 全是「推送失败（第 1 次，完整诊断）」老现象（首次推送
  桥解析失败、0.8 秒后重试成功）；日志超 1MB 已由 `SizeLimitedFileSink` 清空旧内容
  ⇒ 文件只剩后 11 条用例段（开头 15 段的 W 不在文件里，属正常）；
* 三条定稿判据实测全中：r89（`可重复任务=…` + `可重复保留: …翻新商品[0x00224FE8` +
  `ui.select 0x00224FE8` → `idx=110@翻新商品`）；r90 同构（`~0x0002A418` →
  `idx=146@0x202a418@职业杀手`）；r75（`nonav=[…0x9136=（不可导航）深藏不露` +
  `该任务暂无导航目标:深藏不露` 报**原名**）。

### 2. 处置（工具侧，零产品改动）：`plan_regex_audit` 的滚动噪声

* 现象：复跑后跑本工具报 **58 条未命中** —— 全是日志已滚掉的 15 条用例段（段不存在 ⇒
  每条断言都「没跑到」）；残留 11 段（r75~r62）全部命中，不是正则写错；
* 修法（`tools/test/plan_regex_audit.py`）：缺段分两类 —— ① **前缀缺失**（缺的段全部
  早于最早的在段）= 滚动清掉 ⇒ 只提示、不计 bad；② 中间/末尾缺段 = 真异常 ⇒ 仍计 bad；
  零用例段 ⇒ 退出码 2（无从体检，防静默假绿）；
* 验证三例：真日志（15 段滚动提示 + 「全部命中」exit 0）/ 合成「中间缺段」日志（exit 1）/
  零段边界（exit 2）。

### 3. 收口

* 第 89/90/91 轮产物（可重复 20 条 / DLC SFFL / 「（不可导航）」前缀）实机收口；
* 回归：verify 全过（含第 93 轮判据 + 「MO2 部署 == 工作区」）+ 离线层 18 用例 / 920 断言
  + 14 件黄金快照全绿；剩余待办 = **0.1.8 上传包** + 眼睛判据。

---

## 十四·补二十四、第 98 / 99 轮：DLC 链式门槛的两条用例（`r98_dlc_chain` / `_pass`）

> 2026-09-22 第 99 轮：**完整设计 + 实测复盘已合并进 `docs/06` 第九节**（内容不丢，
> 只搬家以控 100 KB 上限；B 段在实测后换了构造：推 `MQ_Shell@100`）。仍然生效的要点：
> ① 两条用例插在 `r62_reload_observe` **之前**、各自自清场（只动这 7 条 DLC 任务）；
> ② A 段断「六条后续**任一**进『链式没到』名单」——空进度近似（`ref/_dlc_info_precheck.py`：
>    六条的对话组都有「可能可用」的那几组）⇒ 不会被 INFO 门槛先藏；名字只写 `[0x`、
>    **不写死运行期 FormID**（DLC 前缀随加载序变，`~0x…` 记录号写法就是为它存在的）；
> ③ B 段断「狂热逾界 / 信念之争 / 发掘过去」**三条都不被链式藏**（MQ_Shell@100
>    是它们共同的启动边）；**不要再推 MQ01@10000** —— 实测造不出状态，且收尾
>    fragment 带 `Self.Stop()` + `Game.AddAchievement(51)`（不可观测 + 成就污染）；
> ④ verify 侧 4 条检查（两个 `[case:]` + A/B 断言形状 + 「不许写死 FormID」与
>    「不许再推 MQ01@10000」两条反向检查）；
> ⑤ ★ **第 100 轮实测：全量 29/29 全 PASS** —— B 段改推一次通过（`assert.nolog` 无命中 +
>   推送 216→219 强佐证）；A 段「藏」+ B 段「放行」双向实证 ⇒ **DLC 收口完成**。
