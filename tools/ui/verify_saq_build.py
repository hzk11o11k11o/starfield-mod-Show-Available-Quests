...
def main() -> int:
    ...
    if dll.exists():
        ...
        if release_mode:
            ...
            all_ok &= gone
            # 反向检查（第 58 轮）：v57 及更早的驱动器版本串不应再出现 —— 日志里那串
            #   「驱动器 v58：…」是「跑的是不是修好时序那版」的唯一判据（同 SWF stamp=）。
            gone = "驱动器 v57".encode() not in blob
            print(("OK  " if gone else "MISS") + " DLL · 旧驱动器版本串 v57 已替换(反向检查)")
            all_ok &= gone
