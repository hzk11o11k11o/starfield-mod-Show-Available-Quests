...
检查的特征（随版本演进补充；旧特征缺失代表功能被回退，不要删）：
  ...
  第 156 轮（**可招募船员入口段落地 + 模拟招募正样本复核** —— 玩家决策：两个都做）：
          ...
          判定断言/五条反向）。
  第 157 轮（**实机检查收口：直读槽位修正 + 两条过时断言更新**）：
          ① 产品缺陷（第 156 轮 r155 的 FAIL 真因）：`SAQ_Crew` 原先调 commonlibsf 的
             `Actor::IsInFaction` —— 其 Actor.h 把该函数标在 vtable 槽 **0x175**，而
             1.16.244 实际在 **0x174**（差 1 槽）⇒ 直读恒 false（4 位真「可招募解锁」
             被错藏）。修复 = 显式按槽 0x174 调用 + **函数头 20 字节指纹校验**
             （换版本 ⇒ 停用直读 + WARN + 保守放行）。
          ② 用例计划两条过时断言更新：r47 `入口=21(…)` → 结构口径 `入口=\d+(…)`
             （45 条后总数随加载状态变化、不可用允许 0~3）；r80 `入口条目表=21 条`
             → `=45 条（任务板 13 + 可重复 NPC 8 + 可招募船员 24）`；r155 模拟招募
             断言 → 直读对照全一致（`不一致 0 位`）+ 判定行存在。
  ...
  第 159 轮补（**r155 复跑 FAIL 的收口 —— 驱动器就绪门** —— 检查 2026-09-25 06:49:29
          会话（`Only=r155` 增量跑）：判据链体检全绿（log_hygiene 退出码 0 / 窗口自检 0 问题 /
          增量筛选只跑 1/45 条），但 r155 的**第一步** `teleport.entry 0x00015062` 就 FAIL：
          「一个候选都取不到：任务板自身[无] 新建常驻 marker[无] 同 cell 常驻兜底1[无]
          同 cell 常驻兜底2[无]」—— 四个候选全 0 = `Masters::MakeFormID` 对未解析 master
          返回 0 ⇒ **静态表还没建**（产品无缺陷：玩家路径 = 打开菜单 ⇒ BuildRuntimeRows
          ⇒ Refresh）。现场：Papyrus `06:49:28 VM is thawing` → 06:49:29 脚本回写通道就绪
          → 驱动器 0.35 秒后开跑；增量跑的第一条用例 ^是^ 本会话第一件事（菜单没开过 +
          存档里没有残留引导 ⇒ 产品的表预建也不跑））：
          ① 驱动器 **v74**：就绪门加「世界已落地（MainMenu/LoadingMenu/FaderMenu 全关）+
             静态表就绪（主动 `Masters::Refresh()`）」两道闸（过不了就等，只记一行日志）；
          ② 三个走静态表的解析器（`teleport.entry` / `~0x…` / `guide.probe`）加防御性
             检查 —— 表没就绪时给「静态表还没就绪…」而不是误报「取不到 / 一个候选都取不到」；
          本脚本检查：驱动器 v74 串（反向 v73）+ 就绪门/等门/静态表主动就绪/静态表未就绪
          四条 DLL 特征串。
...
HARNESS_STRINGS = (
    ...
    ("harness 驱动器版本串",
     "驱动器 v74（第 159 轮补）：就绪门加「世界已落地"),
    # ★★★ 第 159 轮补（驱动器就绪门 —— 06:49:29 会话 r155 的第一步 teleport.entry 报
    #   「一个候选都取不到」：四个候选全 0 = 静态表还没建。产品无缺陷，见 docs/09 十五·6）：
    #   四条特征串钉住 —— 世界落地判定 / 等门文案 / 主动建表 / 表判定失败。发布构建里
    #   整段不编译（SAQ_Test.cpp 被排除），反向检查覆盖。
    ("harness 就绪门世界落地文案", "世界还没落地（"),
    ("harness 就绪门等待文案", "—— 就绪后再开跑（第 159 轮补：通道就绪 ≠ 世界落地/表已建）"),
    ("harness 静态表主动就绪文案", "静态表已主动就绪（本会话第一次用到表"),
    ("harness 静态表未就绪文案", "静态表还没就绪（master 前缀解析失败）"),
    # ★★★ 第 155 轮（可招募船员跟踪研究 · docs/16 16.3）：新 op `crew.probe` ——
    #   24 位可招募船员的**只读**探针（Papyrus faction 三件套 + DLL 直读招募任务状态）。
    #   逐位行 / 汇总行都进 dev DLL ⇒ 发布构建必须一条都不见
    #   （SAQ_Test.cpp 不编译，本表反向检查覆盖）。
    #   ★★★ 第 156 轮扩展：③ 直读对照（`｜直读: ` + `对照=一致/不一致`）+ ④ 模拟招募
    #   往返（`模拟招募：` + 判定文案）—— 同样只在 dev 构建。
    #   ★★★ 第 161 轮（0.1.19 首次发布构建打包，2026-09-25）：原两条「数据表样本名」
    #   （`林监工` / `瓦斯科`）**移出本表** —— 第 156 轮起这两位的显示名进了**产品**
    #   入口表（`SAQ_EntryTable.h` 的 `（可招募）林监工` / `（可招募）瓦斯科`），
    #   子串匹配对「产品长串里含短串」照样命中 ⇒ `verify --release` 误报
    #   「harness 泄漏」把打包挡下（与第 144 轮三条共用契约串同类；实测 = 发布 DLL 里
    #   `（可招募）瓦斯科` 在场、`crew.probe` / `船员探针 {}` 不在场）。替代 = 两条
    #   **只属于探针**的文案（逐位行前缀 / 超时文案）；「数据表在不在」不做字符串判据
    #   （表只有名字 + 数字，名字与产品数据必然重合）——「产品侧船员数据在」由产品表的
    #   名字前缀（`（可招募）玛丽卡·波罗斯`）正向覆盖。
    ...
    ("harness 船员探针逐位行前缀", "船员探针 {}"),
    ("harness 船员探针超时文案", "船员探针没有跑完"),
    ...
    # ★★★ 第 138 轮（P2 会话收口 · 形态隔离）：`ui.mode` —— 运行期强制 UI 形态
    #   （探针用例跑 swf 干净环境 / 产品路径用例跑 auto）。第 137 轮 P2 会话实测：
    #   产品路径（auto）先自动激活并扩 tab ⇒ 探针 `扩tab=fail（8→8）`、断言 `切7`
    #   等到的是 `切8` ⇒ 两条 FAIL（r120 / r133）。修法 = 用例显式设形态 +
    #   产品侧「激活失败不再重试」。dev 正向；发布构建一条都不见（本表反向检查覆盖）。
    ("harness UI 形态覆盖 op 名", "ui.mode"),
    ("harness UI 形态覆盖产品行（可被 assert.log 取证）", "界面形态覆盖：UiMode={}"),
    # ★★★ 第 140 轮（P4 交互接管 · docs/15 十四节）：`ui.interact` —— 接管判据链
    #   （装接管 + 分类 + dry 触发 + 激活拦截）/ 真按键（引导 + 星图交接）/ 只读状态。
    #   实现只在开发构建（SAQ_UiInject.cpp 的 `#if SAQ_WITH_HARNESS` 段 + SAQ_Test.cpp
    #   的 op 名）⇒ 发布 DLL 必须一条都不见（本表反向检查覆盖）。
    ("harness 交互接管 op 名", "ui.interact"),
    ("harness 交互接管探针产品行（可被 assert.log 取证）", "界面接管 {}"),
    ("harness 交互接管按键产品行（可被 assert.log 取证）", "界面接管按键 {}"),
    ("harness 交互接管状态产品行（可被 assert.log 取证）", "界面接管状态 {}"),
    ("harness 交互接管 · 分类判据标记（｜分类=）", "｜分类="),
    ("harness 交互接管 · 触发判据标记（｜触发=）", "｜触发="),
    ("harness 交互接管 · 激活拦截判据标记（｜激活=）", "｜激活="),
    ("harness 交互接管 · dry-run 硬约束（真动作=）", "真动作="),
)
...
UI_INJECT_PRODUCT_STRINGS = [
    ...
    # ★★★ 第 138 轮（P2 会话收口）：激活失败 ⇒ 本菜单周期**不再重试** —— 第 137 轮
    #   P2 会话实测：一旦失败（tab 数不是原版 7 项 ⇒ SetTabsData 永远 +0），150 ms
    #   节流重试会持续刷 WARN（96 条 / 15 秒）+ 每拍全量重建（快照 + 构造 206 条）。
    #   钉住这句防回退（改回「无限重试 + 每条都打日志」⇒ 必 MISS）。
    ("产品 · 激活失败不再重试（本菜单周期收口）", "本次菜单不再重试"),
    # ★★★ 第 140 轮（P4 交互接管 · docs/15 十四节）：**产品路径**（发布也编译）——
    #   按钮劫持 X/Y + activate 拦截 + 引导竖条就地刷新 + 星图交接（真关菜单原语）。
    #   钉住：接管安装失败告警 / 引导下发 / 星图交接 / 委托回原版 / 激活拦下 /
    #   菜单关闭的接管计数行 / 关菜单原语与委托事件名（原版 AS3 契约）。
    ("产品 · 接管安装失败告警", "界面接管：安装失败"),
    ("产品 · 接管 · 引导已下发", "引导已下发（0x"),
    ("产品 · 接管 · 星图交接（真关菜单原语）", "界面接管：星图交接 —— "),
    ("产品 · 接管 · 关菜单原语事件名（Missions）", "ProcessUserEvent(\"Missions\")"),
    ("产品 · 接管 · 委托回原版（X/Y 的 dispatch 复刻）", "已委托回原版"),
    ("产品 · 接管 · 委托事件名（设定航线）", "MissionMenu_PlotToLocation"),
    ("产品 · 接管 · 委托事件名（显示在地图上）", "MissionMenu_ShowItemLocation"),
    ("产品 · 接管 · 激活拦截（主条目拦下）", "展开/收起由原版树逻辑完成"),
    ("产品 · 接管 · 激活拦截事件名", "MissionsList::itemActivated"),
    ("产品 · 接管 · 不可导航不发请求", "没有导航目标（不发请求"),
...
def main() -> int:
    ...
    if dll.exists():
        ...
        for name, needle in {
            ...
            # ★★★ 第 151 轮（排序进日志）：收集完成（排序后）的两行顺序日志 ——
            #   `列表顺序：…`（分段 + 头部名字链 + 入口段位置判定）与
            #   `入口顺序：…`（入口段显示名顺序：任务板段 → 可重复 NPC 段）。
            #   起因 = 第 150 轮的「任务板混进（可重复）堆」此前只能靠眼睛核（玩家
            #   截图）；现在顺序本身可被 assert.log 取证（r80 两条新断言）。
            "列表顺序日志（列表顺序：）": "列表顺序：".encode(),
            "入口顺序日志（入口顺序：）": "入口顺序：".encode(),
            ...
            # ★★★ 第 157 轮（检查收口 · 直读槽位修正）：槽 0x174 的诊断日志 ——
            #   修复依据 = Papyrus native `Actor.IsInFaction` 的注册实现就是
            #   `call [rax+0xBA0]` ⇒ **槽 0x174**（commonlibsf 的 Actor.h 标 0x175 =
            #   差 1 槽，第 156 轮实机代价 4 位不一致/错藏）；直读实现带函数头指纹
            #   校验（换版本 ⇒ 不符 ⇒ 停用 + WARN + 保守放行）。
            "船员直读指纹校验(ok)": "船员直读：槽 0x174 指纹=ok".encode(),
            "船员直读指纹校验(fail)": "槽 0x174 的函数指纹与该版本".encode(),
            "船员直读 vtable 取不到(fail)": "船员直读：Actor vtable 取不到（槽 0x174）".encode(),
        ...
        if release_mode:
            ...
            gone = ("驱动器 v57".encode() not in blob and "驱动器 v58".encode() not in blob and
                    ...
                    "驱动器 v72".encode() not in blob and
                    # ★★★ 第 159 轮补：v73 同理（新版串里带的是「v73 的 ini [Test] Only …」）。
                    "驱动器 v73".encode() not in blob)
            print(("OK  " if gone else "MISS") +
                  " DLL · 旧驱动器版本串 v57~v63/v66/v68/v70~v73 已替换(反向检查)")
            all_ok &= gone
            ...
            if plan_src.exists():
                ...
                # ★★★ 第 155/156/157 轮（可招募船员探针）：步骤 + 断言形状 ——
                ...
                #   ④ ★★★ 第 157 轮**核心判据**：`对照 \d+ 位 / 不一致 0 位` —— 直读槽位
                #      修正（0x175 → 0x174）后 Papyrus vs 直读必须全一致；第 156 轮实机
                #      4 位不一致的回归防线；
                #   ⑤ ★★★ 第 157 轮：模拟招募改为机制记录（只钉 `判定：` 行出现）——
                #      《AddToAvailableCrew 不改变 A 位》已实测（旧断言「A 位随招募往返
                #      0→1→0」不可能成立，见 docs/16 16.9）；
                #   ⑥ 六条反向断言（失败码 / 等回执超时 / 招募任务表单取不到 / 直读不一致 /
                #      模拟招募跳过 / 船员直读指纹停用）= 链路健康判据。
                ...
                all_ok &= check("用例计划 · r155 直读对照全一致断言（第 157 轮核心判据）",
                                plan_text.encode(),
                                "assert.log 船员探针 汇总：.*对照 \\d+ 位 / 不一致 0 位".encode())
                all_ok &= check("用例计划 · r155 模拟招募判定行断言（机制记录）", plan_text.encode(),
                                "assert.log 船员探针 模拟招募：.*判定：".encode())
                bad_r155_old = bool(re.search(
                    r"assert\.(?:log|nolog) 船员探针 模拟招募：\.\*A 位随招募往返", plan_text))
                print(("MISS " if bad_r155_old else "OK  ") +
                      " 用例计划 · r155 旧「A 位随招募往返」断言已换（反向检查，第 157 轮）")
                all_ok &= not bad_r155_old
                ...
                if p2_src.exists():
                    ...
                    for label, needle in (
                        ...
                        #   形态（探针 swf / 产品路径 auto）—— 防「产品先激活、探针撞车」
                        #   回归（第 137 轮实测 r120/r133 两条 FAIL 的真因）。
                        ("P2 探针形态隔离（ui.mode swf）", "step = ui.mode swf".encode()),
                        ("P2 产品路径形态（ui.mode auto）", "step = ui.mode auto".encode()),
                        # ★★★ 第 140 轮（P4 交互接管 · docs/15 十四节）：`r140_interact_takeover`
                        #   —— 按钮劫持 X/Y + activate 拦截（dry-run 判据链）+ 真按键
                        #   （引导 + 星图交接）+ 端到端「星图=开」。
                        ("P2 交互接管用例段头", "[case:r140_interact_takeover]".encode()),
                        ("P2 交互接管探针步骤", "step = ui.interact\n".encode()),
                        ("P2 交互接管 · 接管判据断言",
                         "assert.log 界面接管 .*接管=ok".encode()),
                        ("P2 交互接管 · 四段汇总断言（分类/触发/激活/真动作 同现）",
                         "界面接管 .*分类=ok.*触发=ok.*激活=ok.*真动作=0".encode()),
                        ("P2 交互接管 · 真按键步骤（key X）", "step = ui.interact key X".encode()),
                        ("P2 交互接管 · 真按键判据断言（回调/引导/星图交接）",
                         "界面接管按键 .*按键=X.*回调=1 次.*引导=ok.*星图交接=已调用".encode()),
                        ("P2 交互接管 · 星图入口断言（第 44 轮文案）",
                         "assert.log 星图：界面侧会自己关掉整个暂停菜单".encode()),
                        ("P2 交互接管 · 状态探查步骤", "step = ui.interact state".encode()),
                        ("P2 交互接管 · 端到端断言（星图=开）",
                         "界面接管状态 .*星图=开".encode()),
                    ):
                ...
                #   ① DLL 入口表计数（`入口条目表=45 条（任务板 13 + 可重复 NPC 8 +
                #      可招募船员 24）`；★ 第 110 轮任务板 +1 = 追踪者联盟悬赏信息台；
                #      ★★★ 第 157 轮：21 → 45 = +可招募船员 24 —— 该行是**编译期常量**、
                #      不受引用加载状态影响，是「入口表真的并进了船员段」的最稳判据）；
                ...
                #      Tick 的产品处理打印，窗口基准在个别时序下会落在它之后）。
                all_ok &= check("用例计划 · r80 入口三类计数断言（第 157 轮：45 = 13+8+24）",
                                plan_text.encode(),
                                "assert.log 入口条目表=45 条（任务板 13 \\+ 可重复 NPC 8 \\+ 可招募船员 24） scope=case".encode())
                #   ★ 反向检查只扫 `step =` 行（注释里的历史沿革记载不算回归 ——
                #     第 157 轮首跑就是被注释里的旧口径文案误报成 MISS）。
                bad_r80_old = bool(re.search(r"(?m)^\s*step\s*=.*入口条目表=21 条", plan_text))
                print(("MISS " if bad_r80_old else "OK  ") +
                      " 用例计划 · r80 旧「21 条」断言已换（反向检查，第 157 轮）")
                all_ok &= not bad_r80_old
                #   ★★★ 第 151 轮（排序进日志）：r80 再钉两条 —— 顺序本身的两行
                #   （`列表顺序：共` / `入口顺序：任务板`；子串故意不含数字与括号，
                #   绕开「表条数变化 / 全角括号」那两类假 FAIL）。
                all_ok &= check("用例计划 · r80 列表顺序日志断言（第 151 轮）",
                                plan_text.encode(),
                                "assert.log 列表顺序：共 scope=case".encode())
                all_ok &= check("用例计划 · r80 入口顺序日志断言（第 151 轮）",
                                plan_text.encode(),
                                "assert.log 入口顺序：任务板 scope=case".encode())
                ...
                #   ★★★ 第 157 轮（检查收口）：21 → **45**（+可招募船员 24）后总数随
                #   引用加载状态变化（实测 29/32/33）⇒ 断言改为**结构口径**：
                #   `入口=\d+(可导航 \d+｜marker [1-9]\d* 原板 \d+ 兜底 [1-9]\d* 不可用 [0-3])`
                #   （「不可用」允许 0~3 = 三位 noNav 船员未加载时；实测 1 = 莫亚拉）。
                all_ok &= check("用例计划 · r47 入口统计结构断言（第 157 轮：45 条口径）",
                                plan_text.encode(),
                                "assert.log 入口=\\d+\\(可导航 \\d+｜marker".encode())
                bad_r47_old = "入口=21\\(可导航 21" in plan_text
                print(("MISS " if bad_r47_old else "OK  ") +
                      " 用例计划 · r47 旧「21 条」断言已换（反向检查，第 157 轮）")
                all_ok &= not bad_r47_old
