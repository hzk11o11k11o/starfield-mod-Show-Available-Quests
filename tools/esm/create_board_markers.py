...
def scan_starfield(want_refs: set[int]) -> dict:
    ...
    while pos + 24 <= len(buf):
        ...
        pos += gsize
    return {"boards": boards, "pers_hdr": pers_hdr}


def build_cell_group(items: list[tuple[list, bytes]]) -> bytes:
    """items = [(组链模板, raw 新记录)] → 一棵 `Top 'CELL' > … > CellPersistent` 组树。"""
...
def collect_records(buf: bytes) -> dict[int, dict]:
    """把文件里的 SAQ_BoardMarker_* 记录都收集起来（formid 低 24 位 -> 信息）。"""
    seen: dict[int, dict] = {}
        ...
        while p + 24 <= end:
            ...
            while q + 6 <= len(payload):
                ...
                if sig == b"DATA":
                    data24 = payload[q + 6:q + 6 + n]
                q += 6 + n
            if edid and edid.startswith(MARKER_EDID_PREFIX):
...
def verify(buf: bytes, expect: list[dict], check_data: bool, self_index: int = -1) -> list[str]:
    ...
    for e in expect:
        ...
        if [g for g, _ in got["chain"]][-4:] != [2, 3, 6, 8]:
            problems.append(f"{e['nameZh']} 组链尾部不是 [2,3,6,8]（实际 {[g for g, _ in got['chain']]}）")
...
def main() -> int:
    ...
    cell_group = build_cell_group(items)
