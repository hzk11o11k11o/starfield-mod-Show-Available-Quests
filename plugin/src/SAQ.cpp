...
namespace SAQ
{
	...
	namespace
	{
		...
		//   的任务，「提示」噪音化）。
		//
		//   现在只把**真正会「点了暂时没反应」**的任务算进这一类：
		//   候选池里一个常驻都没有 ⇒ 远处点引导 = 全不可得 = 待生效链路
		//   （结果码 5 + HUD 提示 + 退避重试，靠近后自动生效）。
		//   对比：「首选非常驻 + 池里有常驻候选」的任务 —— 远处点引导会**落到常驻候选**
		//   （立即生效、蓝点在目标附近），靠近后由复算自动升级 → 不需要提示，
		//   也不在这场清单里。
		//   ★★ 第 146 轮：**本机已 0 条** —— 最后 1 条（`SFBGS001_VKaiZ02`，DLC 收集型）
		//   也补上了 world 级常驻兜底（起因 = DLC 的 world 池键与候选 world 口径不一致，
		//   见 gen_guide_targets.py 的 `_world_key`）。模式 6 保留作回归探针：
		//   新内容若再出现「候选全非常驻」的任务，`Mode=6` 一眼筛出来。
		// ------------------------------------------------------------------
		...
		std::string_view TestModeNote(int a_mode)
		{
			...
			case 6: return "只显示「需要靠近」的条目（全部候选都非常驻：远处点引导进入待生效、靠近后自动生效；第 146 轮起本机 0 条，保留作回归探针）";
