...
namespace SAQ
{
	...
	struct QuestEntry
	{
		...
		bool          hasGuideTarget{};
		// ★★ 第 46 轮（大项 B）：这条任务的**全部候选都非常驻** —— 也就是「玩家在远处一定
		//   取不到，必须靠近目标所在区域（其 cell 加载）才能导航」。界面据此在描述里**提前**
		//   告知玩家（「靠近后才可用」），而不是让玩家点了半天看不出反应（实测：「营救机器人」
		//   远处点 R ⇒ 脚本连报 5 次取不到、19 秒后引导被静默放弃）。
		//   判据 = 静态表候选池里没有任何一个 flags bit0（persistent）——
		//   本机数据：209 条有目标的任务里 76 条属于这一类（见 docs/05 第二十节）。
